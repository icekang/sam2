...
def add_prompt(video_label, negative_video_label, predictor, inference_state, annotation_every_n):
    ...
    for i in range(0, video_length):
        ...
        # START Negative Propmts
        negative_mask, _ = load_ann_png(f'{negative_video_label}/{i:05}.png')
        negative_mask_bool = np.all(negative_mask == np.array([255, 0, 0]).reshape(1, 1, 3), axis=2) \
                             | np.all(negative_mask == np.array([0, 255, 0]).reshape(1, 1, 3), axis=2)

        negative_mask_bool = negative_mask_bool & ~mask_bool # remove calcium mask that might be overlapping lumen and wall
        negative_mask_coords = np.argwhere(negative_mask_bool)
        negative_prompts = []
        if len(negative_mask_coords) != 0:
            n_sample_points = min(10,len(negative_mask_coords))
            negative_prompts = np.array(random.choices(negative_mask_coords, k=n_sample_points))
            labels = np.array([0] * n_sample_points, np.int32) # for labels, `1` means positive click and `0` means negative click
            _, out_obj_ids, out_mask_logits = predictor.add_new_points_or_box(
                inference_state=inference_state,
                frame_idx=i,
                obj_id=4,
                points=np.array(negative_prompts),
                labels=labels,
            )
        # END Negative Propmts
        ...
        if i % annotation_every_n != 0:
            ...
            prompts.append({'type': 'point', 'frame': i, 'points': prev_center, 'neg_points': negative_prompts})
        else:
            prev_center = None
            ...
            prompts.append({'type': 'mask', 'frame': i, 'mask': mask_bool, 'neg_points': negative_prompts})
    return prompts
...
def run(model_cfg, sam2_checkpoint, output_name):
    ...
    for volume_path in test_label_dir.glob('*.nii.gz'):
        ...
        negative_video_label = f'/home/gridsan/nchutisilp/datasets/SAM2_Dataset302_Calcium_OCTv2/LaW_nnUNet_all_predictions_Ts/{filename}'
        ...
        prediction_output = Path(f'./{output_name}_annotate_every_{annotation_every_n}_negative_point_prompts/')
